...
class VGGFace2(object):
  ...
  def __init__(
    self, image_dir, image_size, epochs, batch_size, 
    seed=1234, split_ratio=0.0, num_gpus=1):
  ...
  def train_inputs(self):
    ...
    dataset = (dataset.take(self.train_count)
               .shuffle(10000)
               .repeat(epochs)
...
def random_color(image):
  ...
  image = apply_with_random_selector(
      image, lambda x, k: random_color_fast(x, k), num_cases=2)
